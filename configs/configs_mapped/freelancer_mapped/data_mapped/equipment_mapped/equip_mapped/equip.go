...
type Config struct {
	...
	Extras []*Item // accounts for all items not rendered in other tabs
}
...
func Read(files []*iniload.IniLoader) *Config {
	...
	for _, file := range files {
		for _, section := range file.Sections {
			item := &Item{
				Category: strings.ToLower(strings.ReplaceAll(strings.ReplaceAll(string(section.Type), "[", ""), "]", "")),
				Nickname: semantic.NewString(section, cfg.Key("nickname"), semantic.OptsS(semantic.Optional()), semantic.WithLowercaseS(), semantic.WithoutSpacesS()),
			...
			case "[cloakingdevice]":
				...
				frelconfig.Extras = append(frelconfig.Extras, item)
				cloak.Map(section)
			case "[thruster]":
				thruster := &Thruster{
					Nickname:   semantic.NewString(section, cfg.Key("nickname"), semantic.WithLowercaseS(), semantic.WithoutSpacesS()),
