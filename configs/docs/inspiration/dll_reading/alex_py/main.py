...
def parseDLL(fh, out, global_offset):
    ...
    COFF_Head_SizeOfOptionalHeader, = struct.unpack('h', fh.read(2))
...
import config
def getAllInfocards():
    iniData = flinitools.parseDataFile('freelancer.ini', root='EXE')
    resourcesSection = flinitools.getUniqueValue(iniData, 'Resources')
    dllPaths = []
    for entry in flinitools.getValues(resourcesSection, 'DLL'):
        fname = entry[:entry.find(';')].strip()
        realPath = flinitools.handleWindowsFilePath('EXE', fname)
        dllPaths.append(config.flPath + 'EXE' + os.sep + realPath)
