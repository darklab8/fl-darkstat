...
- Your var `CONFIGS_FREELANCER_FOLDER` must point to existing root of a Freelancer game. Which can be Freelancer Vanilla, Discovery or FLSR at this point.
  - using Discovery by default as development freelancer folder is not recommended even if developing for it. Your dev env will boot 5 times faster if using Vanilla Freelancer.
